...
echo "=== FIX2: исправляем дубли start_period в docker-compose.yml ==="
echo "Запущен из $(pwd)"

# 1. Полная остановка (чтобы YAML не блокировала)
docker compose --profile production down --remove-orphans || true

# 2. Создаём чистый docker-compose.yml без дубликатов (с исправлениями)
...
echo "→ docker-compose.yml полностью перезаписан (дубли удалены, postfix переименован, start_period добавлен корректно)"
