...
echo "=== FIX5: ТОЧНАЯ ПРИЧИНА + ПОЛНОЕ ИСПРАВЛЕНИЕ ==="
echo "Запущен из $(pwd)"

echo ""
echo "В ЧЁМ ДЕЛО (честно и по коду):"
echo "1. Ошибка 'service \"app\" depends on undefined service \"worker-payment\"' — потому что app (без profiles) ссылается на worker-payment (с profiles: ['production'])."
echo "   При docker compose ps БЕЗ --profile production Compose считает сервис неопределённым."
echo "2. anonkeymail-app unhealthy — контейнер запустился, но healthcheck (/api/health) не прошёл (либо Prisma не подключился, либо .env кривой, либо Next.js упал)."
echo "   Мы это увидим в логах через 30 секунд после фикса."

echo ""
echo "→ Переписываем docker-compose.yml: убираем лишние depends_on из app (он не должен ждать background-воркеры)"
...
echo "→ docker-compose.yml полностью исправлен (app теперь зависит только от postgres+redis — YAML валидный всегда)"

echo ""
echo "Запуск:"
echo "docker compose --profile production up -d --build"
echo ""
echo "Сразу после запуска пришли мне ВЫВОД этих команд:"
echo "docker compose ps"
echo "docker logs anonkeymail-app --tail=100"
echo "docker logs anonkeymail-postfix --tail=50"
echo "(если app снова unhealthy — логи покажут точную ошибку, починим за 1 шаг)"
