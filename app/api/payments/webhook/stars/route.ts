...
const logger = createLogger("api.payments.webhook.stars");

export async function POST(request: NextRequest) {
  try {
    const rawBody = await request.text();
    const signature = request.headers.get("x-webhook-signature") || request.headers.get("x-stars-signature");
    const secret = process.env.STARS_WEBHOOK_SECRET || process.env.TELEGRAM_BOT_TOKEN;
    ...
    const externalId = body?.externalId || body?.payload || body?.invoice_payload;
    ...
    await captureException(error, { path: "/api/payments/webhook/stars", method: "POST" });
    logger.error("api.payments.webhook.stars.failed", { path: "/api/payments/webhook/stars", method: "POST", error: getErrorMessage(error) });
