...
const logger = createLogger("api.referrals.withdraw");
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (amountUsd < minWithdrawalUsd) {
      return NextResponse.json({ error: `Minimum withdrawal is ${minWithdrawalUsd} USD` }, { status: 400 });
    }

    const availableBalance = Number(user.referralBalance) - Number(reserved._sum.amountUsd || 0);

    if (amountUsd > availableBalance) {
      return NextResponse.json({ error: "Insufficient available referral balance" }, { status: 400 });
    }
    ...
    await captureException(error, { path: "/api/referrals/withdraw", method: "POST" });
    logger.error("api.referrals.withdraw.failed", {
      path: "/api/referrals/withdraw",
      method: "POST",
